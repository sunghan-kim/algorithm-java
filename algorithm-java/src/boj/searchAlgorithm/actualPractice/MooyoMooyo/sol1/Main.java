package boj.searchAlgorithm.actualPractice.MooyoMooyo.sol1;
...
public class Main {
    ...
    private static void down() {
        for (int i = 0; i < 10; i++) {
            ...
            for (int j = 0; j < N; j++) if (M[j][i] != 0) list.add(M[j][i]); // 0이 아닌 값들 임시 리스트(list)에 저장
            for (int j = 0; j < N - list.size(); j++) M[j][i] = 0; // 0이 아닌 값들을 제외한 나머지 값들의 자리에 0으로 채움
            for (int j = N - list.size(); j < N; j++) M[j][i] = list.get(j - (N - list.size()));
        }
    }
